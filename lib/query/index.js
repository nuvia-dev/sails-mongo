...
Query.prototype.parseExpression = function parseExpression(field, expression) {
  ...
  if (_.isPlainObject(expression) && !_.isDate(expression)) {
    return _.reduce(expression, function (obj, val, modifier) {
      ...
      // Handle `equal` by transforming to $eq
      if(modifier === '=' || modifier === 'equal' || modifier.toLowerCase() === 'eq') {
        obj['$eq'] = self.parseValue(field, modifier, val);
        return obj;
      }
...
Query.prototype.parseValue = function parseValue(field, modifier, val) {
  ...
  if(_.isString(val)) {
    ...
    if((!modifier || (modifier !== '=' && modifier !== 'equal' && modifier.toLowerCase() !== 'eq')) && !validator.isMongoId(val) && !this.config.caseSensitive){
